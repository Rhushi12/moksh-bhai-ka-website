...
// Handles all Firebase operations for diamonds and leads
...
export const getDiamondsRealtime = (callback) => {
...
// Add new diamond
export const addDiamond = async (diamondData, imageFile) => {
  try {
    let imageUrl = '';
    ...
    // Ensure name field is present (generate if missing)
    const diamondWithName = {
      ...diamondData,
      name: diamondData.name || `${diamondData.shape} Cut Diamond`,
      image: imageUrl,
      updatedAt: serverTimestamp()
    };
    
    // Add diamond to Firestore
    const docRef = await addDoc(collection(db, 'diamonds'), diamondWithName);
...
// Update diamond
export const updateDiamond = async (diamondId, updateData) => {
  try {
...
// Delete diamond
export const deleteDiamond = async (diamondId, imageUrl) => {
  try {
...
export const getDiamondsByCategory = (category, callback) => {
  const q = query(
...
export const getBestsellerDiamonds = (callback) => {
  const q = query(
    collection(db, 'diamonds'), 
...
export const getLeadsRealtime = (callback) => {
...
// Search leads
export const searchLeads = (searchTerm, callback) => {
...
export const getContactMessagesRealtime = (callback) => {
...
export const searchContactMessages = (searchTerm, callback) => {
...
export const getContactMessagesStats = (callback) => {
  getContactMessagesRealtime((messages) => {
    const stats = {
...
export const getPortfolioStats = (callback) => {
  getDiamondsRealtime((diamonds) => {
    const stats = {
      totalDiamonds: diamonds.length,
      totalValue: diamonds.reduce((sum, diamond) => {
        const price = parseFloat(diamond.price?.replace(/[$,]/g, '') || '0');
        return sum + price;
      }, 0),
      bestsellerCount: diamonds.filter(d => d.bestseller).length,
      categories: diamonds.reduce((acc, diamond) => {
...
export const getLeadsStats = (callback) => {
  getLeadsRealtime((leads) => {
    const stats = {
